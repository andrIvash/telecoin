...
import { getTrendDirection } from './getTrendDirection.js';
...
const isFlagPattern = (prices) => {
    const priceChannelWidth =
        Math.max(...prices.high) - Math.min(...prices.low);
    const priceOpenCloseDistance =
        Math.max(...prices.open) - Math.min(...prices.open);
    // Adjust the threshold based on the average volatility
    const averageVolatility = calculateAverageVolatility(prices);
...
export const getFormattedPrice = (prices, ticksLength) => {
    ...
    for (
        let i = ticksLength ? prices.length - ticksLength : 0;
        i < prices.length;
        i += 1
    ) {
...
export const checkDivergence = (closes, rsiValues, stochasticValues) => {
    ...
    for (let i = 1; i < closes.length; i += 1) {
        ...
        const stochasticValue = stochasticValues[i]
            ? stochasticValues[i].k
            : undefined;

        const previousClose = closes[i - 1];
        const previousRSI = rsiValues[i - 1];
        const previousStochastic = stochasticValues[i - 1]
            ? stochasticValues[i - 1].k
            : undefined;
        ...
        if (
            highPeaks.length >= 2 &&
            highPeaks[highPeaks.length - 1].price >
                highPeaks[highPeaks.length - 2].price &&
            lowPeaks.length >= 2 &&
            lowPeaks[lowPeaks.length - 1].price <
                lowPeaks[lowPeaks.length - 2].price &&
            ...
            console.log(`Increase Divergence found at index ${i},
                prices: ${highPeaks[highPeaks.length - 2].price} - ${
                highPeaks[highPeaks.length - 1].price
            }`);
        }
        ...
        if (
            ...
            prices: ${lowPeaks[lowPeaks.length - 2].price} - ${
                lowPeaks[lowPeaks.length - 1].price
            }`);
        }
    }
